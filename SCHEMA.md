...
CREATE INDEX idx_usage_log_created_at ON metrics(created_at);
CREATE INDEX idx_usage_log_request_id ON metrics(request_id);
CREATE INDEX idx_usage_log_api_key_created ON metrics(api_key_id, created_at);
```

## Redis Schema (Future High-Performance Layer)

- Redis serve as the primary data store for `metrics` and `quota` table.
- Postgres is backup data store for `metrics` and `quota` table.
- whenever a request comes, update `metrics` and `quota` in primary data store.
- these changes are synced to backup data store.

### Quota Table

```redis
# Key Pattern: quota:{api_key_id}:{service_id}
# Value: Redis Hash. {"avaiable": 20, "consumed": 20, "updated_at": 1200, "service_account":}
# TTL: persist forever
quota:123:456 → {"avaiable": 20, "consumed": 20, "updated_at": 1200}
quota:123:457 → {"avaiable": 0, "consumed": 20, "updated_at": 1200}
```

### Metrics Table
```redis
# Pattern: metric:{api_key_id}:{service_id}
# Value: Redis Hash. bucket:{timestamp} -> number of request in that bucket
# TTL: bucket:{ts} expired in 2 days, to avoid key space explosion. it can be shortened manually.
usage_buffer:123:456 → {"buket:0": "23", "bucket:120": "12"}
```
...
```sql
SELECT s.name, q.available, q.consumed
FROM quotas q
