...
// returns keyID (int64).
func (as *AdminService) AddKeyToUser(ctx context.Context, user *User, apiKey string) (*APIKey, error) {
	// Start transaction
	...
	_, err = qtx.UpdateAPIKeyStatus(ctx, &dbsqlc.UpdateAPIKeyStatusParams{
		ID:     apiKeyRecord.ID,
		Status: "assigned",
	})
	if err != nil {
